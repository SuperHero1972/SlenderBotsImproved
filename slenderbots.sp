...
public Plugin:myinfo = {
    name = "SlenderBots Improved",
    author = "SuperHero1972",
    description = "Allows TFbots to play SF2. Original by TehPlayer14, Improved by SuperHero1972",
    version = "1.5",
    url = ""
};
...
#define SCARE_SPRINT_SPEED 190.0
#define SCARE_SPRINT_TOLERANCE 8.0
#define SCARE_SPRINT_HOLD_TIME 1.5

new bool:g_IsBotSprinting[MAXPLAYERS + 1];
new Float:g_BotSprintUntil[MAXPLAYERS + 1];
...
{
    if (!IsValidEntity(page)) return false;

    decl String:strName[64];
    GetEntPropString(page, Prop_Data, "m_iName", strName, sizeof(strName));

    // Avoid the model reference entity and the spawnpoint base
    if (StrEqual(strName, "sf2_page_model", false)) return false;
    if (StrContains(strName, "sf2_page_spawnpoint", false) != -1) return false;

    // Any page-related entity (often just "sf2_page###") is fine
    if (StrContains(strName, "sf2_page", false) != -1) return true;
...
stock TeleportFlagToExit()
{
	...
	while((i5 = FindEntityByClassname(i5, "trigger_multiple")) != -1)
	{
		decl String:strName[50];
		GetEntPropString(i5, Prop_Data, "m_iName", strName, sizeof(strName));
		if(StrContains(strName, "sf2_escape_trigger", false) != -1)
		{
			GetEntPropVector(i5, Prop_Send, "m_vecOrigin", position);
			TeleportEntity(FLAGT, position, NULL_VECTOR, NULL_VECTOR);
			break;
...
// Grab only the X/Y speed of bots for scare-sprint detection.
stock Float:GetClientHorizontalSpeed(client)
{
...
stock bool:IsBotInScareSprintSpeed(client)
{
    new Float:speed = GetClientHorizontalSpeed(client);
    return speed >= (SCARE_SPRINT_SPEED - SCARE_SPRINT_TOLERANCE) && speed <= (SCARE_SPRINT_SPEED + SCARE_SPRINT_TOLERANCE);
}

stock StopBotSprint(client)
{
    if(g_IsBotSprinting[client])
    {
        FakeClientCommand(client, "-sprint");
        g_IsBotSprinting[client] = false;
    }
}

stock UpdateBotScareSprint(client)
{
    if(!IsValidClient(client) || !IsPlayerAlive(client) || GetClientTeam(client) != _:TFTeam_Red || !IsFakeClient(client))
    {
        if(client > 0 && client <= MaxClients)
        {
            g_BotSprintUntil[client] = 0.0;
            StopBotSprint(client);
        }
        return;
    }
    ...
    if(IsBotInScareSprintSpeed(client))
        g_BotSprintUntil[client] = now + SCARE_SPRINT_HOLD_TIME;

    if(now <= g_BotSprintUntil[client])
    {
        if(!g_IsBotSprinting[client])
        {
            FakeClientCommand(client, "+sprint");
            g_IsBotSprinting[client] = true;
        }
    }
    else
    {
...
public OnEntityDestroyed(ent)
{
    ...
    if (StrEqual(cls, "monster_generic", false))
    {
        UpdateBlockers();
    }

    // SAFELY check entity index before using it on arrays
    if (0 <= ent < sizeof(IsTargetedPage))
    {
...
stock LookAtTarget(any:client, any:target)
{
    ...
    {
        return; // target is not valid, skip everything
    }
